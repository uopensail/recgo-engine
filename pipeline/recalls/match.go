...
type Matcher struct {
	...
	program *minia.Minia                // Compiled expression program
}
...
func NewMatcher(conf *model.MatchRecallConfigure) *Matcher {
	pStat := prome.NewStat("NewMatcher")
	defer pStat.End()
...
func (m *Matcher) Do(uCtx *userctx.UserContext) model.Collection {
	pStat := prome.NewStat("Matcher.Do")
	defer pStat.End()

	if m.index == nil {
		pStat.MarkErr()
		zlog.LOG.Error("Matcher.Do.NoIndex", zap.String("recall", m.conf.Name))
		return nil
	}
	...
	if result == nil {
		pStat.MarkErr()
		zlog.LOG.Warn("Matcher.Do.NoResult", zap.String("recall", m.conf.Name))
		return nil
	}
	...
	if err != nil {
		pStat.MarkErr()
		zlog.LOG.Error("Matcher.Do.GetStringsError", zap.Error(err))
		return nil
	}
	zlog.LOG.Info("Matcher.Do.KeysExtracted", zap.Int("count", len(keys)))
