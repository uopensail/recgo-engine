...
def item_filed_schema():
    ...
    meta = {
        ...
        "i_s_level": (FeatureType.Int64Type, lambda i:  levels[random.randint(0, 100) % len(levels)]),
        "i_s_score": (FeatureType.Float32Type, lambda i:  scores[random.randint(0, 100) % len(scores)]),
        "i_s_language": (FeatureType.StringType, lambda i:  langs[random.randint(0, 100) % len(langs)]),
        "i_s_country": (FeatureType.StringType, lambda i:  ctrys[random.randint(0, 100) % len(ctrys)]),
        "i_s_cat1": (FeatureType.StringType, lambda i:  cat1s[random.randint(0, 100) % len(cat1s)]),
        "i_s_cat2": (FeatureType.StringsType, lambda i:  random.sample(cat2s, k=2)),
        "i_d_ctr": (FeatureType.Float32Type, lambda i:  random.randint(0, 100)/100.0),
        "i_s_cat": (FeatureType.StringType, lambda i:  cats[random.randint(0, 100) % len(cat2s)]),
...
class Pool:
    ...
    def __init__(self, count, dir):
        ...
        subpool = gen_subpool(self.itemdf, self.items_feature,
                              ["i_s_language='en'", "i_s_level=2 and i_d_ctr > 0.5"])
        subpool_filedata = {}
        subpoolid = 1
        ...
        InvertIndex(dir, self.itemdf, schema, [
                    "i_s_language", "i_s_level", ["i_s_country", "i_s_cat"]])
    ...
    def gen_meta(self, schema):
        ret = {}
        ret['field_data_type'] = {
            'u_s_country': FeatureType.StringType, 
            'u_s_cat': FeatureType.StringsType,
            'u_s_language': FeatureType.StringType
        }

        for k, v in schema.items():
            ret['field_data_type'][k] = v[0]
        return ret
